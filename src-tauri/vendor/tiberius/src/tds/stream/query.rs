    ...
    pub(crate) async fn forward_to_metadata(&mut self) -> crate::Result<()> {
        loop {
            ...
            match item {
                ...
                // Bakehouse patch: leave the tokens we surface as QueryItems in
                // the stream instead of consuming them here.
                Some(ReceivedToken::Info(_)) | Some(ReceivedToken::Error(_)) => break,
                Some(ReceivedToken::Done(d))
                | Some(ReceivedToken::DoneInProc(d))
                | Some(ReceivedToken::DoneProc(d))
                    if d.has_count() =>
                {
                    break
                }
    ...
    pub async fn into_results(mut self) -> crate::Result<Vec<Vec<Row>>> {
        ...
        while let Some(item) = self.try_next().await? {
            match (item, &mut result) {
                ...
                // Bakehouse patch: collecting APIs ignore message tokens.
                _ => {}
            }
        }
...
pub enum QueryItem {
    ...
    /// Bakehouse patch: an informational message from the server
    /// (PRINT output, RAISERROR with severity <= 10, STATS messages).
    Info(crate::tds::codec::TokenInfo),
    /// Bakehouse patch: a server error token (severity > 10). The stream also
    /// terminates with this error at EOF; receiving it here lets a client show
    /// every error with its line number as it happens.
    Error(crate::tds::codec::TokenError),
    /// Bakehouse patch: a DONE token that carried a row count
    /// ("N rows affected").
    RowsAffected(u64),
}
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            ...
            return match token {
                ...
                // Bakehouse patch: surface info/error/done tokens instead of
                // discarding them, so a workbench can build a messages pane.
                ...
                | ReceivedToken::DoneProc(done)
                    if done.has_count() =>
                {
                    Poll::Ready(Some(Ok(QueryItem::RowsAffected(done.rows()))))
                }
                _ => continue,
            };
        }
